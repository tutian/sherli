        ...
        if (data) {
            if (data.data) {
                ...
                datas.forEach(function(item) {
                    ...
                    _html += '<a data-url="' + GOODLIST_DETAIL + '&goods_id=' + item.id + '" class="goods_list_item">';
                    _html += '<div class="goods-top"><span class="goods-img-box"><img data-lazyload="' + item.img + '"></span></div>';
                    ...
                    _html += '<p class="goods-price"><span class="goods-price-num">¥' + item.sell_price + '</span><span class="goods-git">送' + item.sesame + '芝麻币</span></p>';
                    _html += '<p class="goods-maket-price">市场价:<span class="goods-maket-price-num">¥' + item.market_price + '</span></p>';
                    _html += '</div></a>';
                    li.innerHTML = _html;
            ...
            mui('.goods-disc').on('tap','a',function(){
                ...
                setTimeout(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            var nohistory = document.querySelector(".nohistory");
                            ...
                            if(data.data.length<20&&currentPage>1){
                                currentPage=0;
                                flag=true;
                            }
...
$(function() {
    ...
    var asideCreateDom = function(goods, id) {
        ...
        $(".aside-sure-btn").on("tap", function() {
            ...
            window.location.replace(LOAD_GOODS_LIST + params);
        });
    ...
    var navDom = function(id) {
        mui(".goodslist-nav-bar").on("tap", "i", function() {
           ...
           if ($(this).hasClass("goodslist-default")) {
                window.location.replace(GOODLIST_DEFAULT + params);
                //              goodslistMainAjax("", 0);
            }
            if ($(this).hasClass("goodslist-sell")) {
                window.location.replace(GOODLIST_SELL + params);
                //              goodslistMainAjax("", 1);
            }
            if ($(this).hasClass("goodslist-hot")) {
                window.location.replace(GOODLIST_HOT + params);
                //              goodslistMainAjax("", 2);
            }
            if ($(this).hasClass("goodslist-price")) {
                window.location.replace(GOODLIST_PRICE + params);
                //              goodslistMainAjax("", 4);
            }
        });
    };
