...
(function($, window, document, name) {
	...
	$.createLoadMask = function(callback) {
		var element = document.createElement('div');
		element.classList.add(CLASS_LOADMARSK);
		element.addEventListener($.EVENT_MOVE, $.preventDefault);
		element.innerHTML = '<div class="load-content"><i></i> 正在加载...</div>';
//		element.addEventListener('tap', function() {
////			mask.close();
//		});
		var load_mask = [element];
		load_mask._show = false;
		load_mask.show = function() {
			load_mask._show = true;
			element.setAttribute('style', 'opacity:1');
			document.body.appendChild(element);
			return load_mask;
		};
		load_mask._remove = function() {
			if (load_mask._show) {
				load_mask._show = false;
